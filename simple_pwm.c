...
volatile int brake_on_counter;
volatile int brake_off_counter;
...
{
    float percent = constrain(value, -1, 1);
    int direction = (percent > 0); // set direction pin to 1 for positive
...
void read()
{
    ...
    if (brake_on_counter > 10) { 
        brake_state = 1; 
    } else if (brake_off_counter > 10) {
        brake_state = 0;
    }
}

void setBrake(int goal_state) {
    if (goal_state == 1 && brake_position < 4000) {
        brake_output = 1.0;
...
int main()
{
    ...
    while (true)
    {
        ...
        if (iters > 1000) {
            ...
            // printf("Throttle: %f\n", throttle);
            // printf("[REMOTE] Steer: %f\tBrake: %f\tThrottle: %f\n", steer, brake, throttle);
            printf("[OUT] Steer: %f\tBrake: %f\tThrottle: %f\n", steer_output, brake_output, last_throttle);
            iters = 0;
        } else {
            iters += 1;
        }
